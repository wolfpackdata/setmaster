"""Stage 4 — Traktor-Spotify Playlist Join.

Verbatim port of SM2 `traktor_spotify_playlist_join.py`.
Reads the stage-3 compare CSV, and for every row with Status == 'both'
joins the Spotify (Exportify) CSV against the Traktor playlist CSV and
the full collection, writing <work_dir>/Joined/joined_<playlist>.csv.

The matching heuristics here (clean_track_name transliteration map /
NFKD / punctuation / term removal / ' feat' truncation, hash_id dedup,
format_traktor_key text map, presence_flag logic, collate columns,
track_collate sort) are copied verbatim — do not modify.

Plumbing changes vs SM2: the Exportify directory is a parameter; paths
derive from work_dir. Per-playlist failures are recorded as warnings and
processing continues (same as SM2, which counted errors and exited 0).
"""
...
def run_stage4(work_dir, exportify_dir):
    """Process all 'both' playlist pairs from the compare CSV.

    Returns (outputs, warnings). Raises if the compare CSV is unreadable.
    """
    work_dir = Path(work_dir)
    mapping_file = work_dir / "traktor_spotify_playlist_compare.csv"
    traktor_full_collection_file = work_dir / "Traktor" / "traktor_collection_tracks.csv"
    traktor_dir = work_dir / "Traktor"
    spotify_dir = Path(exportify_dir)
    output_dir = work_dir / "Joined"
    ...
    try:
        ...
        msg = f"Failed to process collection file: {e}; continuing without collection file paths"
        warnings.append(msg)
        print(f"  WARNING: {msg}")
    ...
    for idx, row in mapping_df.iterrows():
        ...
        if not os.path.exists(traktor_path):
            msg = f"[{playlist_name}] Traktor file not found: {traktor_filename}"
            warnings.append(msg)
        ...
        if not os.path.exists(spotify_path):
            msg = f"[{playlist_name}] Spotify file not found: {spotify_filename}"
            warnings.append(msg)
        ...
        try:
            ...
            msg = f"[{playlist_name}] Failed to process playlist pair: {e}"
            warnings.append(msg)
            print(f"  ERROR: Failed to process playlist pair: {e}")
            error_count += 1
