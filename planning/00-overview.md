...
**Positioning — job 1 does not depend on job 2** (clarified 2026-07-31, #208; not a change of scope, a statement of what was always true). **Set preparation is the primary function and it stands alone.** SM3 is fully useful with **no collection ever loaded**: set rows are typed by hand (`01-data-model.md` §4.2 — BPM/Key are manually typed, never auto-filled; `in_name` carries no FK to a collection track), and the collection-dependent screens (S3, S5) show their §7.1 empty state until a pipeline run exists. Catalog analysis is the **optional** second half — Traktor Pro and Exportify CSVs are inputs to *that* half only, and the sole thing a loaded collection adds to S2 is typeahead name suggestions. A DJ who never touches the catalog tools still gets the whole reason SM2 existed: a purpose-built transition-row editor instead of a spreadsheet. **Never present Traktor as a prerequisite** — in UI copy, README, install notes, or store/repo descriptions.
